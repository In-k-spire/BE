...
interface CategoryJpaRepository: JpaRepository<Category, Long> {
    @Query("select c from Category c join fetch c.bookList join fetch c.user where c.id = :id")
    fun findByIdOrNull(id: Long): Category?;
    @Query("select c from Category c join fetch c.bookList where c.user = :user")
